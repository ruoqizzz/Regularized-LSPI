...
def test_agent4LQR2D(agent, env, gamma, ifshow=False):
	# states = np.linspace(-10,10,500)
	...
	now = time.strftime("%Y-%m-%d",time.localtime(time.time()))
	now2 = time.strftime("%H_%M_%S",time.localtime(time.time()))
	path = "data/LQR2D/"+now+"/"+str(agent.opt)+"-"+str(agent.reg_param)+"-BF"+str(agent.policy.basis_func.size())+'-'+agent.policy.basis_func.name()+"/"+now2+"/"
	folder = os.path.exists(path)
	if not folder:
		os.makedirs(path)   
	fn = path+"policy.pickle"
	...
	# plot policy
	plt.plot(states[:,1], actions_estimate)
	# print(actions_true)
	plt.xlabel('states')
	plt.ylabel('actions')
	plt.ylim(-3,3)
	plt.plot(states[:,1], actions_true)
	plt.legend(('estimate', 'true'))
	# plt.show()
	pltfn = path+"policy.png"
	# f = open(fn, 'wb')
	...
	n_episode = 100
	for i in range(n_episode):
		agent_state_his.append(np.array(state.reshape(1,2))[0])
		optimal_state_his.append(np.array(state_o.reshape(1,2))[0])
	...
	plt.plot(np.array(agent_state_his)[:,1], label='estimate')
	plt.plot(np.zeros(n_episode), 'k--')
	plt.plot(np.array(optimal_state_his)[:,1], label='optiomal')
	# print(actions_true)
	plt.ylabel('state')
	plt.xlabel('episode')
	plt.legend(loc='upper right')
	...
	pltfn = path+"action2.png"
	# f = open(fn, 'wb')
	plt.savefig(pltfn,dpi=300)
	if ifshow:
		plt.show()
	fn = path+"action_his2.pickle"
	f = open(fn, 'wb')
	pickle.dump(agent_state_his, f)
	f.close()
