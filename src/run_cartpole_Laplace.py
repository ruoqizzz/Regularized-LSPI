...
def main():
	...
	parser.add_argument('--reg_param', default=0.02, type=float)
	...
	for i_s in range(len(rbs)):
		...
		for i_test in range(10):
			...
			path = "data/CartPole/"+now+"/"+str(agent.opt)+"-"+str(agent.reg_param)+"-BF"+str(agent.policy.basis_func.size())+'-'+agent.policy.basis_func.name()+'-episodes'+str(num[i_s])+"/"+now2+"/"
