...
class LQREnv(object):
	...
	def step(self, action):
		# non linear
		u = action
		if np.linalg.norm(u) > 1*u.shape[0]*u.shape[1]:
			action = np.matrix([1.]*u.shape[0]*u.shape[1]).reshape(u.shape[0],u.shape[1])
	...
	def true_Qvalue(self, L, gamma, state, action):
		...
		# non linear
		if np.linalg.norm(u) > 1*u.shape[0]*u.shape[1]:
			u = np.matrix([1.]*u.shape[0]*u.shape[1]).reshape(u.shape[0],u.shape[1])
