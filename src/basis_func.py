...
class RBF(object):
	...
	def __init__(self, input_dim, n_features, n_actions, gamma):
		...
		self.feature_means = [[np.random.uniform([-1.5, -3, -np.pi/12, -3], [1.5, 3, np.pi/12, 3]) for _ in range(self.n_features-1)] for _ in range(self.n_actions)]
	...
	def evaluate(self, state, action):
		...
		diff = state - self.feature_means[action]
		rbf = np.exp(-self.gamma*np.sum(diff*diff, axis=1))
		phi[offset] = 1.
